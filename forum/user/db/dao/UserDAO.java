...
public class UserDAO implements DAO<User, String> {
    ...
    @Override
    public boolean updateOnline(@NotNull final String login, boolean isOnline) {
        boolean result = false;
        try (PreparedStatement statement = connection.prepareStatement(UserQueries.UPDATE_ONLINE.QUERY)) {
    ...
    @Override
    public boolean isOnline(@NotNull final String login) {
        boolean result = false;
        try (PreparedStatement statement = connection.prepareStatement(UserQueries.GET_ONLINE.QUERY)) {
            statement.setString(1, login);
            ResultSet resultSet = statement.executeQuery();
    ...
    @Override
    public int getAmountOfOnlineUsers() {
        int amountOfOnline = 0;
        try (PreparedStatement statement = connection.prepareStatement(UserQueries.GET_AMOUNT_OF_ONLINE.QUERY)) {
            ResultSet resultSet = statement.executeQuery();
            if(resultSet.next()) {
                amountOfOnline = resultSet.getInt(1);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return amountOfOnline;
    }
    ...
    enum UserQueries {
        ...
        UPDATE("UPDATE logintable SET password = sha1(?) WHERE login = ?"),
        DELETE("DELETE FROM logintable WHERE login = ? AND password = sha1(?)"),
        UPDATE_ONLINE("UPDATE logintable SET online = ? WHERE login = ?"),
        GET_ONLINE("SELECT online FROM logintable WHERE login = ?"),
        GET_AMOUNT_OF_ONLINE("SELECT count(login) FROM logintable WHERE online = true");
